...
- [ ] 1.1 Check `VONDirect` spelling and `WCS_Armament_ConfirmLock` FilterPreset against a real in-game config (rebind the key in game, read the saved `InputUserSettings`); record the result in a comment on the new rows
- [ ] 1.2 Check whether the ripple-quantity action is `TurretNextRippleQuantity` or `TurretWeaponNextRippleQuantity` and note the answer in the issue
