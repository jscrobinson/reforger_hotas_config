...
- [x] 2.2 Add `WCS_Armament_ConfirmLock` to `WCS_ACTIONS`; verify it appears only when WCS actions are ticked (confirmed working by the maintainer)
- [x] 2.3 Correct the `VONDirectToggle` hint; verify the wording in the browser (wording confirmed by the maintainer)
...
- [x] 3.1 Bind each new action, download the config, and verify the `.conf` has an `Action` block per name
- [x] 3.2 Load that config back and verify every binding reappears; also load a vanilla template and verify nothing changed
